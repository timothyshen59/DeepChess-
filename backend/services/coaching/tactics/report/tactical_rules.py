...
def select_motif(
...
) -> tuple[TacticalCategory, TacticalMotif]:
    ...
    if defense and defense.largest_immediate_capture_cp >= SIGNIFICANT_MATERIAL_LOSS_CP:
...
def confidence_for(
    ...
    has_forced_mate = (
        ...
        or (features.best_move is not None and features.best_move.gives_checkmate)
        or (
            features.played_defense is not None and features.played_defense.opponent_has_mate_in_one
        )
    )

    has_large_material_loss = (
        features.played_defense is not None
        and features.played_defense.largest_immediate_capture_cp >= SIGNIFICANT_MATERIAL_LOSS_CP
    )
